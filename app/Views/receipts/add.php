...
<div class="container pb-5">
    <div class="row mt-3 justify-content-start">
        <div class="col">
            <a href="<?php echo URLROOT; ?>/accounts/show/<?php echo $data['account_id']; ?>" class="btn btn-secondary me-2"><i class="bi bi-arrow-left me-2"></i>Back to Account</a>
            <a href="<?php echo URLROOT; ?>/receipts/index/<?php echo $data['account_id']; ?>" class="btn btn-secondary"><i class="bi bi-file-earmark-text me-2"></i>Back to Receipts</a>
        </div>
    </div>
    <div class="row">
    ...
    <form action="<?php echo URLROOT; ?>/receipts/add/<?php echo $data['account_id']; ?>" method="post" id="receiptForm">
        <div class="row">
            <div class="col-sm-12 col-md-8 col-lg-6 mx-auto">
                ...
                <div class="mb-3">
                    ...
                    <input type="datetime-local" class="form-control <?php echo (!empty($data['receipt_date_time_err'])) ? 'is-invalid' : ''; ?>" id="receipt_date_time" name="receipt_date_time" required value="<?php echo htmlspecialchars($data['receipt_date_time']); ?>">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="payer_id" name="payer_id">
                        <?php foreach ($data['users_in_account'] as $user) : ?>
                            <option value="<?php echo htmlspecialchars($user->id); ?>" <?php echo ($user->id == $_SESSION['user_id']) ? 'selected' : ''; ?>>
                    ...
                    <table class="table table-borderless">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td width="80">
                                    <div class="mb-0">
                                        <input oninput="this.value = this.value.toUpperCase(); updateItemCurrencies(this.value);" type="text" class="form-control text-center" id="currency" name="currency" list="currenciesDatalist" required value="<?php echo htmlspecialchars($data['default_account_currency']); ?>">
            ...
            <div class="col-sm-12 col-md-8 col-lg-6 mx-auto">
                ...
                <div class="text-center d-flex justify-content-end">
                    <button type="submit" class="btn btn-success" id="createReceiptBtn" disabled><i class="bi bi-receipt me-2"></i>Create Receipt</button>
                </div>
            </div>
        </div>
    </form>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="confirmReceiptModalLabel">Confirm Receipt Creation</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p>Are you sure you want to create this receipt with the following details?</p>
                <div id="receiptPreview" class="receipt-preview">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
                <button type="button" class="btn btn-primary" id="confirmYes">Yes, create</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    const R_ACCOUNT_ID = <?php echo $data['account_id']; ?>;
    const R_CURRENT_USER_ID = <?php echo isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'null'; ?>;
    let R_DEFAULT_CURRENCY = "<?php echo htmlspecialchars($data['default_account_currency']); ?>";
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const dateTimeInput = document.getElementById('receipt_date_time');
        ...
        document.getElementById('confirmYes').addEventListener('click', function() {
            ...
            // Construct the JSON payload from the form data
            const payload = {
                location: formData.get('location'),
                receipt_date_time: formData.get('receipt_date_time'),
                totalAmount: formData.get('total_amount'),
                payer_id: formData.get('payer_id'),
                currency: formData.get('currency'),
                note: formData.get('receipt_note'),
                items: []
            };

            const itemElements = receiptForm.querySelectorAll('.accordion-item');
            itemElements.forEach((item, index) => {
                const itemName = item.querySelector(`input[name="item_name[${index}]"]`).value;
                const itemPrice = item.querySelector(`input[name="item_price[${index}]"]`).value;
                const beneficiaries = Array.from(item.querySelectorAll(`input[name="bought_for[${index}][]"]:checked`)).map(cb => cb.value);
            ...
            fetch(`<?php echo URLROOT; ?>/receipts/add/<?php echo $data['account_id']; ?>`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success && data.redirect_url) {
                    ...
                    confirmBtn.innerHTML = 'Yes, create';
                }
            })
            .catch(error => {
    ...
    function updateItemCurrencies(newCurrency) {
        ...
        if (document.getElementById('confirmReceiptModal') && document.getElementById('confirmReceiptModal').classList.contains('show')) {
            populateReceiptPreview();
        }
    }
    ...
    function attachGlobalEventListeners() {
        ...
        if (currencyInput) currencyInput.addEventListener("input", function() {
    ...
    // AKORDEON BAŞLIĞINI GÜNCELLEME FONKSİYONU (FİYAT BİLGİSİ İLE)
    function updateAccordionHeader(nameInputElement) {
        ...
        const idParts = nameInputElement.id.split('_'); // örn: item_name_itemDetail_0
        const currentItemUniqueId = idParts.slice(2).join('_'); // örn: itemDetail_0
    ...
    // FİYAT DEĞİŞTİĞİNDE BAŞLIĞI GÜNCELLEMEK İÇİN YARDIMCI FONKSİYON
    function updateHeaderFromPriceChange(priceInputElement) {
        const accordionItem = priceInputElement.closest('.accordion-item');
        if (!accordionItem) return;

        const idParts = priceInputElement.id.split('_'); // örn: item_price_itemDetail_0
        const currentItemUniqueId = idParts.slice(2).join('_'); // örn: itemDetail_0
    ...
    function addItem() {
        ...
        if (R_ACCOUNT_USERS && R_ACCOUNT_USERS.length > 0) {
            R_ACCOUNT_USERS.forEach(user => {
                const isChecked = parseInt(user.id) === parseInt(R_CURRENT_USER_ID);
                ...
                <div class="accordion-body">
                    <div class="row">
                            ...
                            <div class="row mb-3 align-items-end">
                                <div class="col">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control text-center flex-grow-0" style="width: 70px;" id="amount_${currentItemUniqueId}" name="amount[${itemIndex}]" value="1" min="1" required oninput="this.value = Math.max(1, parseInt(this.value)) || 1;">
                            ...
                            <div class="row mb-3">
                                 <div class="col">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" step=".01" class="form-control" id="item_price_${currentItemUniqueId}" name="item_price[${itemIndex}]" placeholder="Price" required oninput="updateHeaderFromPriceChange(this)">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div id="bought_for_error_${currentItemUniqueId}" class="text-danger mt-1" style="display: none;">Lütfen en az bir kişi seçin.</div>
                        </div>
                    </div>
                </div>
            </div>`;
    ...
    document.getElementById('totalAmount').addEventListener('input', calculateTotalAmount);
    document.getElementById('receiptForm').addEventListener('input', function(event) {
        if (event.target.matches('input[name^="item_price["]') || event.target.matches('input[name^="amount["]')) {
            calculateTotalAmount();
        }
        // Also re-calculate total if currency changes
